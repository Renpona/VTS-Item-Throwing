...
wsClient.on('connect', function (connection) {
    ...
    connection.send(auth.checkForCredentials("Renpona", "VTuber Ball Physics"));
});
...
function parseResponse(response, connection) {
    ...
    else if (response.messageType == "Live2DParameterListResponse") {
        eventEmitter.emit("parameterListReceived", response.data);
        //console.log(response.messageType);
    }
    else if (response.messageType == "InjectParameterDataResponse") {
        eventEmitter.emit("parameterUpdateComplete");
    }
}
...
function createParamBatch(paramArray) {
    return utils.buildRequest("InjectParameterDataRequest", {"parameterValues": paramArray});
}
...
module.exports = {
    ...
    createParamBatch: createParamBatch,
